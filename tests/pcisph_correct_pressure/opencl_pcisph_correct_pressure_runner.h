...
public:
  ...
  run(const PcisphCorrectPressureCase &tc) override {
    ...
    std::vector<float> pressureInit(input.pressure.begin(),
                                    input.pressure.end());
    cl::Buffer pressureBuf(
        opencl.context(), CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
        pressureInit.size() * sizeof(float), pressureInit.data(), &err);
    if (err != CL_SUCCESS) {
    ...
    cl::Buffer rhoBuf(opencl.context(), CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
    ...
    if (opencl.queue().enqueueReadBuffer(
            pressureBuf, CL_TRUE, 0, sizeof(float) * N,
